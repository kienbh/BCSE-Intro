...
> ✅ **A8 · BCSE Pulse CÔNG KHAI — XONG (25/8, LIVE):** `https://sv05.bcse-vju.com/` (root 302 → `/pulse/`). Trang tĩnh Agri-OS đọc Kuma heartbeat API (`/api/status-page/heartbeat/bcse`, status page slug `bcse` 16 monitor) + `pulse.json` (cron `7 * * * *` trên VM105: psql `ops_umami_db` → views/sessions 7 ngày từng cổng, chỉ số liệu tổng hợp an toàn). nginx: `/pulse/*`, `/status/*`, `/api/status-page/*`, `/assets/*` public — `/overview`, `/dashboard` vẫn sau SSO (verify cả hai chiều). Files trong `Server Management\sv05-ops-center\`: `pulse\index.html`, `pulse_query.sql`, `pulse_gen.sh`, `create_pulse_status_page.py` (tunnel SSH → Kuma :3006), `deploy_pulse_public.py` (redeploy = chạy lại script này). Verify 25/8: 16/16 cổng up, 541 views/98 phiên 7d.
