...
| 🟡 **VÁ XONG** | **sv08 bcse-intro** | Vá nhanh ✅ · Migrate token ⏳ | *(30/8)* **Đã vá nhanh** — light mode hết vỡ: map đủ mọi sắc slate còn sót (`slate-950/55,80`, `900/40,60,70`, `800/30,50`, `800` đặc, `700`, `black/20`, `text-slate-700`); hạ tông 11 nhóm accent pastel; thêm `.on-media` giữ chữ trắng cho Hero + avatar gradient + nút solid; CTA→section sáng. Build OK, 117 rule `html.light` trong CSS đã build. **Còn lại (P1):** migrate token đầy đủ theo §5 để bỏ hẳn blocklist. |
