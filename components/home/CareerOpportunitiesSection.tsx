    ...
    <section id="careers" className="section-padding">
      <div className="container-max">
        ...
        <ScrollReveal>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-12">
            {[
              ...
              <div key={i} className="p-4 rounded-xl bg-surface-2/40 border border-line/[0.06] text-center">
                <s.icon className="w-5 h-5 text-sky-400 mx-auto mb-2" />
                <p className="text-sm font-bold text-ink mb-0.5">{s.value}</p>
                <p className="text-[10px] text-ink-5 leading-tight">{s.label}</p>
              </div>
            ))}
          </div>
        </ScrollReveal>
        ...
        <ScrollReveal delay={0.05}>
          <h3 className="text-lg font-bold text-ink mb-2">{t('careers.byTrack')}</h3>
          <p className="text-xs text-ink-5 mb-6">{t('careers.byTrackNote')}</p>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-12">
            {trackCareers.map((tr) => (
              <div key={tr.specId} className="p-5 rounded-xl bg-surface-2/40 border border-line/[0.06] hover:border-sky-500/20 transition-colors">
                <h4 className="text-sm font-bold text-ink mb-3">{pickLocalized(tr.name, lang)}</h4>
        ...
        <ScrollReveal delay={0.1}>
          <h3 className="text-lg font-bold text-ink mb-2">{t('careers.integrated')}</h3>
          <p className="text-xs text-ink-5 mb-6 max-w-3xl">{t('careers.integratedNote')}</p>
          {!showIntegrated && (
            <div className="mb-12">
              <button
          ...
          <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-12 ${showIntegrated ? '' : 'hidden'}`}>
            {integratedDirections.map((dir) => {
              ...
              return (
                <div key={dir.id} className={`p-5 rounded-xl bg-surface-2/40 border ${c.border} hover:bg-surface-2/60 transition-colors`}>
                  <div className={`inline-flex items-center justify-center w-10 h-10 rounded-xl ${c.bg} mb-3`}>
                    <Icon className={`w-5 h-5 ${c.text}`} />
                  </div>
                  <h4 className="text-sm font-bold text-ink mb-0.5">{pickLocalized(dir.name, lang)}</h4>
                  <p className={`text-[11px] font-mono ${c.text} mb-2`}>{dir.nameEN}</p>
                  <p className="text-xs text-ink-4 leading-relaxed mb-3">{pickLocalized(dir.description, lang)}</p>
                  <div className="flex flex-wrap gap-1">
                    {dir.roles.map((r) => (
                      <span key={r} className="text-[10px] px-2 py-0.5 rounded-full bg-fill/5 text-ink-4">{r}</span>
                    ))}
                  </div>
                </div>
              );
          ...
          <div className="p-8 rounded-2xl bg-gradient-to-r from-sky-600/15 to-indigo-600/15 border border-sky-500/20">
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
              ...
              <div className="flex-1">
                <h4 className="text-base font-bold text-ink mb-1">{t('careers.portalTitle')}</h4>
                <p className="text-sm text-ink-4">{t('careers.portalDesc')}</p>
              </div>
              <Link
                href="/services#career-portal"
                className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold bg-sky-500 hover:bg-sky-400 text-ink rounded-xl transition-colors whitespace-nowrap"
              >
