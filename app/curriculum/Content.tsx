    ...
    <div className="pt-20">
      <section className="section-padding">
        <div className="container-max">
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-semibold rounded-xl border transition-colors ${
                  fw === key
                    ...
                    : 'bg-slate-800/40 border-white/[0.06] text-slate-500 hover:text-slate-300'
                }`}
              >
          ...
          <p className="text-xs text-slate-600 text-center mb-16">{ps.partnerNote} · {ps.englishLevel}</p>
          ...
          {fw === '135' ? (
            <>
              ...
              <div className="flex flex-wrap gap-2 mb-16">
                {coreRequired135.map((c) => (
                  <span key={c.code} className="px-3 py-1.5 text-xs rounded-lg bg-slate-800/60 border border-white/[0.06] text-slate-300">
                    ...
                    {c.name}
                    <span className="text-slate-500"> · {c.credits} TC</span>
                  </span>
                ))}
              </div>

              <h3 className="text-xl font-bold text-white mb-2">{t('curriculum.modules135')}</h3>
              <p className="text-xs text-slate-500 mb-4">{t('curriculum.modules135Note')}</p>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-16">
                {modules135.map((m) => (
                  <div key={m.id} className="p-5 rounded-xl bg-slate-800/40 border border-white/[0.06]">
              ...
              <h3 className="text-xl font-bold text-white mb-6">{t('curriculum.projects135')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-16">
                {projects135.map((p) => (
                  <div key={p.code} className="p-5 rounded-xl bg-slate-800/40 border border-purple-500/20">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-bold text-white">{p.title}</span>
                      <span className="text-[11px] font-mono text-purple-400">{p.sem}</span>
                    </div>
                    <p className="text-xs text-slate-500">{p.desc}</p>
                    <p className="text-[10px] font-mono text-slate-600 mt-2">{p.code}</p>
                  </div>
                ))}
              </div>

              <h3 className="text-xl font-bold text-white mb-6">{t('curriculum.grad135')}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-16">
                {graduation135.map((g) => (
                  <div key={g.code} className="p-5 rounded-xl bg-slate-800/40 border border-emerald-500/20">
                    <div className="flex items-center justify-between mb-2">
